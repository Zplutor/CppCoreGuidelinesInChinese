...
C和C++是有紧密联系的语言。它们都源自1978年的“古典C”，并且从那之后在ISO委员会中不断发展。为了让它们保持兼容，人们做了很多努力，但它们都不是对方的子集。

C准则概要：

* CPL.1: 优先使用C++而不是C
* CPL.2: 如果你必须使用C，使用C和C++的通用子集，并且以C++来编译C代码
* CPL.3: 如果你必须使用C作为接口，在调用这些接口的代码中使用C++

### CPL.1: 优先使用C++而不是C

##### 理由

C++提供了更好的类型检查和更加符号化的支持。它为高级编程提供了更好的支持，而且通常会生成更快的代码。
...
void* pv = &ch;
int* pi = pv;   // 非C++
*pi = 999;      // 重写&ch附近的sizeof(int)个字节
```

在C中与`void*`之间隐式转型的规则是微妙的，而且没有强制实施。实际上，这个例子违反了转换成更严格对其要求的准则。
...
这个子集可以同时被C和C++编译器编译，而且以C++编译时，会比“纯C”有更好的类型检查。

##### 示例

```cpp
int* p1 = malloc(10 * sizeof(int));                      // 非C++
int* p2 = static_cast<int*>(malloc(10 * sizeof(int)));   // 非C，C风格的C++
int* p3 = new int[10];                                   // 非C
int* p4 = (int*) malloc(10 * sizeof(int));               // 既是C也是C++
```

##### 实施

* 如果使用的构建模式是以C来编译代码，标记出来。
    * C++编译器会强制要求代码是有效的C++，除非你使用了C扩展选项。

### CPL.3: 如果你必须使用C作为接口，在调用这些接口的代码中使用C++

##### 理由

C++比C更有表达力，而且为许多形式的编程提供了支持。

##### 示例

举个例子，为了使用第三方的C库或者C系统接口，应以C和C++的通用子集来定义低级接口来获得更好的类型检查。尽可能把低级接口封装在一个遵循C++指南的接口中（为了更好的抽象，内存安全，以及资源安全），并且在C++代码中使用这个C++接口。
...
区分声明（用作接口）和定义（用作实现）。使用头文件来表示接口以及强调逻辑结构。

源文件准则概要：

* SF.1: 如果你的项目没有遵守其它规范，那么为代码文件使用`.cpp`后缀，为接口文件使用`.h`
...
* SF.10: 避免依赖于隐式`#include`包含的名称
* SF.20: 使用`namespace`来表达逻辑结构
* SF.21: 不要在头文件中使用没有名称（匿名）的名称空间
* SF.22: 为所有内部的/不导出的实体使用没有名称（匿名）的名称空间

### SF.1: 如果你的项目没有遵守其它规范，那么为代码文件使用`.cpp`后缀，为接口文件使用`.h`

##### 理由

这是一个存在已久的规范。但是一致性更重要，所以如果你的项目使用了其它规范，应遵守它。

##### 注意

这个规范反映了一个常见的使用模式：头文件更常与C共享，同时以C++和C编译，而后者一般使用`.h`，而且把所有头文件命名为`.h`比起仅仅为了那些要与C共享的头文件而使用不同的扩展名更简单。另一方面，实现文件很少与C共享，所以一般应该与`.c`文件区分，所以通常最好把所有C++实现文件命名成其它名称（例如`.cpp`）。

这里指定的名称`.h`和`.cpp`不是强制的（只是建议作为默认选择），其它名称也有广泛使用。例如`.hh`、`.C`和`.cxx`。应等价地使用这些名称。在本文档中，我们使用`.h`和`.cpp`作为头文件和实现文件的简略表达方式，即使实际的扩展名也许不一样。

你的IDE（如果你使用了的话）可能有关于后缀的选项。
...
如果在程序中`#include <foo.h>`出现了两次，你会得到两个违反了“单一定义规则”的链接错误。
...
这样做会丢失`#include`有效消除歧义以及使用其它替代物的能力。也会让`#include`头文件有了顺序依赖，因为当它们以不同的顺序包含时会有不同的含义。
...
// bad.h
#include <iostream>
using namespace std; // 不好的

// user.cpp
#include "bad.h"

bool copy(/*... 一些参数 ...*/);    // 一些恰好命名为copy的函数

int main() {
    copy(/*...*/);    // 现在重载了局部的::copy和std::copy，会出现歧义
}
```

##### 实施

标记出在头文件全局作用域中的`using namespace`。
...
为了避免包含保护的冲突，不要只是用文件名来命名保护。要确保同时包括一个关键的、具有良好区分度的标识，例如库的名称，或者头文件所属组件的名称。
...
标记出没有使用`#include`保护的`.h`文件。

##### 注意

一些实现提供了第三方扩展，例如`#pragma once`，作为包含保护的另一种选择。这不是标准的，也不是可移植的。除了将你与第三方锁定在一起之外，它还会将宿主机器的文件系统语法注入到你的程序中。我们的建议是按照ISO C++来写代码：参阅P.2准则。

### SF.9: 避免在多个源文件之间出现循环依赖

##### 理由

循环依赖难以理解，并且减慢编译速度。这种复杂应转换成使用语言支持的模块（当它们可用的时候）。

##### 注意

消除循环依赖；不仅仅是用`#include`保护来打破它。
...
避免意外。避免当`#include`包含的头文件改变时，必须修改其它的`#include`。避免无意中依赖于包含在头文件中的实现细节和逻辑上分离的实体。
...
void use()                  // 不好的
{
    string s;
    cin >> s;               // 没问题
    getline(cin, s);        // 出错：getline()未定义
    if (s == "surprise") {  // 出错：== 未定义
        // ...
    }
}
```

`<iostream>`暴露了`std::string`的定义（“为什么？”这是一个有趣的小问题），但是它并不要求包含整个`<string>`头文件来做到这个事情，结果导致了著名的初学者问题“为什么`getline(cin,s);`不能用？”，或者甚至是偶尔出现的“`string`不能用`==`比较”。

解决方法是显式地`#include <string>`：
...
一些头文件只是用来收集一组来自于各种头文件的始终不变的声明。例如：
...
使用者现在可以通过单个`#include`取得这组声明：

```cpp
#include "basic_std_lib.h"
```

这个针对隐式包含的准则并不意味着阻止这种有意的聚合。

##### 实施

实施需要一些知识，了解一个头文件中的哪些东西是意图“导出”给使用者的，以及有什么东西可以用来实现。直到我们拥有模块之前，没有真正好的解决方法。
...
外部的东西都不能依赖于内嵌的、未命名的名称空间中的实体。考虑把一个实现源文件中的所有定义都放进未命名的名称空间中，除非它定义的是一个“外部的/导出的”实体。

##### 示例

API类以及它的成员不能存在于未命名的名称空间；但是任何定义于一个实现源文件中的“辅助”类或者函数应该放在未命名的名称空间作用域中。
...
只使用原始语言的话，做任何东西都是冗长乏味的（任何语言都是如此）。使用了合适的库之后，任何东西都会变得合理地简单。

标准库已经经过了多年稳定的发展。它现在在标准中的描述比语言特性还要长。所以，这部分关于库的指南很可能最终发展成其它部分一样长或者更长。
...
节省时间。不要重复发明轮子。不要重复其他人的工作。当其他人的工作有改进的时候，能够获得好处。在你做出改进的时候帮助其他人。

### SL.2: 优先使用标准库而不是其它库

##### 理由

更多人了解标准库。它更可能保持稳定，有良好的维护，比你自己的代码或者其它大部分库更加广泛可用。

### SL.3: 不要往`std`名称空间添加非标准实体

##### 理由

往`std`添加东西可能会改变其它符合标准的代码的含义。`std`中额外的东西可能会与标准未来的版本产生冲突。
...
有可能做到，但是杂乱的，而且有可能在不同平台下出现问题。

### SL.4: 以类型安全的方式使用标准库

##### 理由

显然，不遵守这个准则会导致未定义行为、内存损坏、以及任何种类的其它糟糕的错误。

##### 注意

这是一个半哲学的元准则，需要很多具体的准则来支撑。我们需要它作为其它更具体准则的保护伞。
...
* SL.con.1: 优先使用STL`array`或`vector`来代替C数组
* SL.con.2: 优先使用STL`vector`作为默认选择，除非你有其它理由使用不同的容器
* SL.con.3: 避免边界错误
* ???
...
C数组更不安全，相比`array`和`vector`没有优势。对于定长数组，应使用`std::array`，它不会在传递给函数的时候衰退成指针，并且能够知道它的长度。而且，像内置数组一样，在栈上分配的`std::array`会在栈上保持它的元素。对于变长数组，应使用`std::vector`，它可以改变长度，以及处理内存分配。
...
std::array<int, SIZE> w;             // 没问题
```

##### 示例

```cpp
int* v = new int[initial_size];     // 不好的，持有原始指针
delete[] v;                         // 不好的，手动删除

std::vector<int> w(initial_size);   // 没问题
```

##### 注意

对于容器中的无所有权引用，使用`gsl::span`。

##### 注意

比较在栈上分配的定长数组与在自由存储上分配元素的`vector`的性能是虚伪的。你同样也可以比较在栈上的`std::array`与通过指针访问的`malloc()`的返回结果。对于大部分代码，栈分配与自由存储分配的区别并不重要，但`vector`的便利与安全却很重要。在这个区别很重要的代码中工作的人们完全可以在`array`和`vector`之间选择。

##### 实施

* 标记出在函数或类中声明的C数组，在该函数或类中同时声明了STL容器（为了避免在遗留的非STL代码中造成过多的干扰警告）。修复方法：至少把C数组修改成`std::array`。

### SL.con.2: 优先使用STL`vector`作为默认选择，除非你有其它理由使用不同的容器

##### 理由

`vector`和`array`是仅有的具有以下优势的标准库容器：提供了最快的通用访问（随机访问，包括对矢量化友好的），最快的默认访问模式（从头到尾或从尾到头是对预读器友好的），以及最低的空间开销（毗邻的布局在每个元素上都是零开销，这是对缓存友好的）。你一般需要从容器中添加或移除元素，所以应默认使用`vector`；如果你不需要修改容器的长度，使用`array`。

即使当其它容器看起来更合适的时候，例如使用`map`来获得O(log N)的查找性能，或者使用`list`来获得高效的中间插入，使用`vector`一般仍然会比大了几KB的容器执行得更好。

##### 注意

`string`不应该用来作为独立字符的容器。`string`是一个文本字符串；如果你想要字符的容器，应使用`vector</*char_type*/>`或者`array</*char_type*/>`。

##### 例外

如果你有好的理由使用其它容器，那就使用它。例如：

* 如果`vector`符合你的需求，但是你不需要变长的容器，那么使用`array`代替。
* 如果你想要一个字典形式的查找容器，来确保O(K)或O(log N)的查找性能，该容器会更大（大几KB），而且你会频繁执行插入，导致维护排好序的`vector`不可行，这时可以使用`unordered_map`或者`map`来代替。
...
* SL.str.12: 为期望定义成标准库`string`的字符串字面量使用`s`后缀
...
使用`char*`来表示一个指向并非字符的东西会引起困惑，并且阻止有价值的优化。
...
```cpp
auto pp1 = make_pair("Tokyo", 9.00);         // {C风格字符串,double} 是期望的吗？
pair<string, double> pp2 = {"Tokyo", 9.00};  // 有点啰嗦
...
`iostream`是用于流式I/O的，类型安全的、可扩展的、有格式化和无格式化的IO库。它支持多种（以及用户可扩展的）缓冲策略和多种语言区域。它可以用于传统的I/O，读取和写入内存（字符串流）、以及用户定义的扩展，例如网络间的流（asio：还未标准化）。

IO流准则概要：

* SL.io.1: 只当你必须使用字符级别的输入时才使用
* SL.io.2: 在读取的时候，总是要考虑不规范的输入
* SL.io.3: 优先使用IO流进行I/O
* SL.io.10: 除非你使用`printf`族函数，否则应调用`ios_base::sync_with_stdio(false)`
* SL.io.50: 避免使用`endl`
* ???
...
除非你真的只是在处理单个的字符，否则使用字符级别的输入会导致用户代码容易出错，并且会导致低效的字符令牌组成。
...
错误一般最好尽早处理。如果输入并非有效，每个函数都必须写成要处理坏数据（这是不切实际的）。
...
`complex`是一个用户定义类型，在定义它的I/O时不需要修改`iostream`库。
...
**讨论**：`iostream`与`printf()`族函数

人们经常（而且经常正确地）指出`printf()`族函数相比`iostream`有两个优势：格式化的灵活性和性能。这必须要与`iostream`的优势进行权衡：处理用户定义类型的扩展性、对违反安全的适应性、隐式内存管理以及`local`的处理。

如果你需要I/O性能，你几乎总是可以做得比`printf()`更好。

`gets()`、`scanf()`使用`s`，而`printf()`使用`%s`，这是一个安全风险（容易受到缓冲区溢出的影响，以及通常容易出错）。在C11中，它们被更安全的函数`gets_s()`、`scanf_s()`和`printf_s()`代替，但是它们仍然不是类型安全的。

##### 实施

有选择性地标记出`<cstdio>`和`<stdio.h>`。

### SL.io.10: 除非你使用`printf`族函数，否则应调用`ios_base::sync_with_stdio(false)`

##### 理由

将`iostream`与`printf`风格的I/O进行同步开销很大。`cin`和`cout`默认是与`printf`同步的。
...
`endl`操作符基本上等同于`'\n'`和`"\n"`；作为最常见的用法，它只会通过执行冗余的`flush()`来减慢输出的速度。与`printf`风格的输出相比，这个降低的速度是明显的。

##### 示例

```cpp
cout << "Hello, World!" << endl;    // 两次输出操作，以及一次刷新
cout << "Hello, World!\n";          // 一次输出操作，没有刷新
```

##### 注意

对于`cin`/`cout`（以及等价物）的交互，没有理由要去刷新；这是自动做的。对于写入到文件，很少有需要刷新。

##### 注意

除去（偶尔重要的）性能问题，在`'\n'`和`endl`之间的选择几乎总是审美问题。

## SL.regex: 正则表达式

`<regex>`是标准C++的正则表达式库。它支持各种正则表达式模式规则。

## SL.chrono: 时间

`<chrono>`（定义于`std::chrono`名称空间）提供了`time_point`和`duration`的概念，以及以各种单位输出时间的函数。它提供了用于记录`time_point`的时钟。
...
### SL.C.1: 不要使用setjmp/longjmp

##### 理由

`longjmp`忽略了析构函数，因此会使所有依赖RAII的资源管理策略失效。

##### 实施

标记出所有出现`longjmp`和`setjmp`的地方。

# A: 架构理念

这部分包含关于更高层级的架构理念和库的思想。

架构准则概要：

* A.1: 分离代码中的稳定部分和不稳定部分
* A.2: 把可能可重用的部分表示成库
* A.4: 在库之间不应该出现循环引用
* ???
* ???
...
库是声明和定义的合集，视为一个整体来维护、文档化和分发。库可以是头文件的集合（“只有头文件的库”）或者是一系列头文件加上一系列目标文件。库可以静态或动态地链接到程序中，或者可能只是用`#include`包含即可。
...
这部分内容包含了在其它地方受欢迎的准则和指南，但是我们特意不建议使用它们。我们很清楚这些准则在某些时候和某些地方是合理的，并且我们自己有时也会使用这些准则。但是，在我们建议、并且由本指南支持的编程风格的语境下，这些“伪准则”会有坏处。

即使在今天，这些准则也在某些语境下也可能是合理的。例如，缺少合适的工具支持会使异常不适宜用于硬实时系统，但是请不要盲目地相信“平庸的智慧”（例如，没有证据支持的关于“效率”的声明）；这些“智慧”可能基于几十年前的信息，或者来自于与C++有很大属性差别的语言的经验（例如C或Java）。

对这些伪准则的替代方案的正面讨论会作为“替代方案”部分列入准则中。

伪准则概要：

* NR.1: 不要：把所有声明都放在函数的顶部
* NR.2: 不要：在函数中只有一个`return`语句
* NR.3: 不要：不使用异常
* NR.4: 不要：把每个类声明放在它自己的源文件中
* NR.5: 不要：不在构造函数中做实质性的事情；使用两步初始化
* NR.6: 不要：把所有清理行为放在函数的底部，然后使用`goto exit`
* NR.7: 不要：把所有数据成员定义成`protected`
* ???
...
这个准则是老式编程语言的遗留物，在这些语言中，不允许变量和常量在语句之后初始化。这样做会导致更长的程序，以及更多由未初始化和错误初始化的变量导致的错误。
...
在未初始化变量和使用点之间的距离越大，出现缺陷的机会也越大。幸运的是，编译器会捕捉许多“在设置之前使用”的错误。不幸的是，编译器不能捕捉所有这些错误，而且缺陷并不总是像这个小例子那样容易定位。
...
单一返回准则会导致不必要曲折的代码，以及引入额外的状态变量。尤其是，单一返回准则更加难以在函数的顶部执行集中的错误检查。
...
这不仅更长，而且可能会更低效。函数越长和越复杂，处理方法会越痛苦。当然，许多简单的函数自然而然地只有一个`return`，因为它们的内部逻辑更简单。
...
{
    if (!p) return -1;  // 错误指示：可以选择“throw nullptr_error{}”
    // ... 执行查找来找到p的索引
    return i;
}
```

如果我们使用这个准则，我们会得到像下面这样的代码：
...
注意，我们（有意地）违反了未初始化变量的准则，因为这种风格经常导致这个问题。同样，这种风格也是使用goto exit伪准则的诱因。
...
我们没有办法让所有人都满意地解决这个问题。毕竟，关于异常的讨论已经持续了40多年。有些语言不能在缺少异常的情况下使用，而有的语言则不支持异常。这会导致使用异常与不使用异常的根深蒂固的传统，并且引起争论。

但是，我们可以简要地概述为什么我们认为异常是通用编程以及在这份指南的语境下的最好选择。赞成或反对的简单争论往往是没有结论的。有些特定的应用程序的确不适合使用异常（例如，缺少对异常处理的开销进行可靠评估的硬实时系统）。

逐个来考虑对异常的主要异议：

* 异常不高效：与什么做对比？当进行比较的时候，要确保处理的是相同的错误组合，并且它们的处理方式是一致的。尤其是，不要把看到错误就立即终止的程序与小心地清理资源然后记录错误的程序进行对比。是的，有些系统的异常处理实现很弱；有时候，这些实现强迫我们使用其它错误处理方法，但这并不是异常的根本问题。当使用效率作为论据时——在任何环境下——要注意你要有好的数据来支撑，通过这些数据能真正地看出正在讨论的问题。

* 异常导致泄露和错误。异常不会导致这些问题。如果你的程序是一个到处充斥指针而没有统一的资源管理策略的老鼠窝，不管你做什么都会有问题。如果你的系统由几百万行这种代码组成，你很可能不能使用异常，但这是滥用以及无规则使用指针的问题，而不是异常的问题。在我们看来，你需要使用RAII来让基于异常的错误处理简单和安全——比其它方法更简单和更安全。

* 异常的性能不可预测。如果你是在一个硬实时系统中开发，你必须保证在给定的时间内完成任务，那么你需要工具来作为这些保证的后盾。据我们了解这类工具还不可用（至少对大部分程序员来说并非如此）。

许多——也许是大部分——关于异常的问题源自于要与混乱的旧代码交互的历史需要。
...
* 异常用于报告错误（在C++中；其它语言对于异常有不同的用法）。
* 异常不是用于可以局部处理的错误。
* 不要试图在每个函数中捕捉每个异常（这是乏味、笨拙的，并且会导致运行缓慢的代码）。
* 异常不是用于发生了不可恢复错误后需要立即终止的模块/系统中的错误。
...
* RAII
* 契约/断言：使用GSL的`Expects`和`Ensures`（直到我们拥有了契约的语言支持）

### NR.4: 不要：把每个类声明放在它自己的源文件中

##### 理由（不要遵守这个准则）

这样做导致生成的很多文件难以维护，并且会降低编译速度。单独一个类通常不是好的维护和分发逻辑单元。
...
* 使用名称空间来包含逻辑上内聚的类和函数集合。

### NR.5: 不要：不在构造函数中做实质性的事情；使用两步初始化

##### 理由（不要遵守这个准则）

遵守这个准则会导致更弱的不变式，更复杂的代码（必须处理半构造的对象），以及错误（我们不能始终正确地处理半构造对象）。
...
`goto`是容易出错的。这个技术是在异常出现之前，类似RAII的资源和错误处理技术。
...
* 使用异常和RAII
* 对于非RAII资源，使用`finally`。

### NR.7: 不要：把所有数据成员定义成`protected`

##### 理由（不要遵守这个准则）

`protected`数据是错误的来源。`protected`数据会被来自于多个地方的不受数量限制的代码操作。`protected`数据是在类层次结构中全局数据的等价物。
...
* 使成员数据为`public`或（最好是）`private`

# RF: 参考资料

有许多为C++而写的编码标准、准则以及指南，特别是为了C++的特定用法。它们很多都：

* 聚焦于低层级的问题，例如标识符的拼写
* 由C++新手编写
* 把“阻止程序员做不平常的事情”作为它们的主要目标
* 把多个编译器（有些是10年前的）下的可移植性作为目标
* 编写来维护几十年前的代码库
* 以单一的应用领域作为目标
* 完全事与愿违
* 被忽略（为了让程序员工作得更好，必须被忽略）

不好的编码标准比没有编码标准更糟糕。然而一份合适的指南集比没有标准好多了：“有规范即解放”。

为什么我们不能拥有一个允许所有我们想要的而不允许所有我们不想要的语言呢（“一个完美的语言”）？从根本上来说，这是因为你用得起来的语言（以及它们的工具链）同时也在服务需求与你不同的人，服务比你需要的更多的需求。而且，你的需求会随着时间改变，一个通用语言可以允许你去适应。在今天看起来理想的语言，在明天可能就会变得过于有限制性。

编码指南对特定的需求调整了语言的用法。因此，不存在对每个人都适用的单个编码风格。我们希望不同的组织提供额外的指南，带有更多的限制和更严格的风格准则。
